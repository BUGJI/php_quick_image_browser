> 优化到尽头了，此项目不打算再维护，有很多烂尾功能
> 我开发了简洁的桌面端软件，请移步 [image_browser](https://github.com/BUGJI/image_browser)
> 如果你愿意分享，请改用Openlist或者其他渠道分享图片，此站不做过多维护
...
- **🗂 AI 分类** —— 左侧 AI分类 虚拟文件夹,按常见场景(天空/几何/树木/花朵/水晶…)浏览,支持实时/预生成索引三档模式(见下节)
...
### 查询词向量缓存
同一关键词/类别描述只调一次 txt2vec,结果缓存在 `.ai_query_cache.json`(自动生成,最多 500 条),
避免频繁搜索触发向量服务频率限制。
...
在左侧目录树顶部显示 **AI分类** 虚拟文件夹,二级就是常见场景(天空云彩、几何、树木、花朵、水晶、人物、星空…),
点开即展示该场景的图片(无需复制文件,直接指向 webp_cache 缓存)。

### 三档模式(`AI_CLASSIFY_MODE`)
| 模式 | 行为 | 说明 |
|------|------|------|
| `off` | 不启用 | 左侧不显示 AI分类(默认) |
| `realtime` | 点击时实时计算 | 每次点击对全库算 topK,无需预生成,首次稍慢 |
| `cache` | 预生成索引(推荐) | 秒开;增量同步时自动只重新分类新增图片 |

### 使用流程(cache 模式)
1. `.env` 设置 `AI_CLASSIFY_MODE=cache`(可选 `AI_CLASSIFY_TOP=200` 每类上限、`AI_CLASSIFY_MIN_SCORE=0` 相似度阈值)
2. 管理面板 `admin.php` → 「🗂 AI 分类」→ 「⚙️ 生成分类索引」(或 SSH: `php ai_vector.php build-index`)
3. 刷新网站,左侧出现 **AI分类** 文件夹,点击展开选择场景

类别清单在 `ai_categories.json`(类别名 + 英文描述,可自行增删;描述质量直接影响分类效果)。
`realtime` 模式无需第 2 步。分类/搜索共用同一向量库,**不需要额外建立图片向量缓存**。
...
| `ai_vector.php` | AI 向量缓存引擎 + 搜索接口(全量/增量建向量,CLI/Web 双模式;build-index 生成分类索引) |
| `ai_vector_lib.php` | AI 公共库(配置/双存储/路径反推/向量服务调用/余弦搜索/查询缓存/分类索引) |
| `ai_categories.json` | AI 分类类别清单(类别名 + 英文描述,可编辑) |
| `env.php` | 轻量 .env 加载器 |
